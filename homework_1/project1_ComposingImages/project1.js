...
function composite(bgImg, fgImg, fgOpac, fgPos) {
    ...
    for (let y = 0; y < fgHeight; ++y) {
        for (let x = 0; x < fgWidth; ++x) {
            ...
            const fgA = fgData[fgIdx + 3];

            const bgR = bgData[bgIdx];
            const bgG = bgData[bgIdx + 1];
            const bgB = bgData[bgIdx + 2];
            const bgA = bgData[bgIdx + 3];

            const totA = fgA + (1 - fgA) * bgA;

            // Alpha Blending
            bgData[bgIdx + 0] = (fgA * fgR + (1 - fgA) * bgA * bgR) / totA; // Red
            bgData[bgIdx + 1] = (fgA * fgG + (1 - fgA) * bgA * bgG) / totA; // Green
            bgData[bgIdx + 2] = (fgA * fgB + (1 - fgA) * bgA * bgB) / totA; // Blue
            bgData[bgIdx + 3] = totA;
        }
    }
}
